...
export function getMailboxUsageBytes(email) {
  ...
  const body = JSON.stringify([['mailboxStatus', { user: email, field: 'vsize', mailboxMask: '*' }, 'usage']])
      ...
      (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            resolve(result.reduce((sum, row) => sum + (Number(row.vsize) || 0), 0))
          } catch {
            resolve(null)
          }
        })
