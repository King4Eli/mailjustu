...
export function AliasesModal({ aliases, onClose, onCreate, onDelete }: AliasesModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
      ...
      >
        ...
        <p className="mb-3 text-xs" style={{ color: 'var(--text-faint)' }}>
          Mail sent to an alias lands in this inbox, and you can send as any of them from Compose.
        </p>
        ...
        <div className="flex flex-col gap-1">
          {aliases.map((a) => (
            ...
            >
              <span>{a.source}</span>
              <button onClick={() => onDelete(a.id)} style={{ color: 'var(--text-faint)' }}>
                <Trash2 size={14} />
              </button>
            </div>
          ))}
          {aliases.length === 0 && (
            <p className="py-2 text-center text-sm" style={{ color: 'var(--text-faint)' }}>
              No aliases yet.
            </p>
          )}
        </div>
