    ...
    return Response.json({
      domains: rows.map(({ dkim_selector, dkim_public_key, ...row }) => ({
        ...
        dnsRecords: buildDnsRecords(
          row.name,
          dkim_public_key
            ? { selector: dkim_selector, publicKey: dkim_public_key }
            : null,
        ),
      })),
      defaults: {
        maxMailboxesPerDomain:
          Number(process.env.MAX_MAILBOXES_PER_DOMAIN) || null,
        maxAliasesPerMailbox:
          Number(process.env.MAX_ALIASES_PER_MAILBOX) || null,
...
export async function POST(req: NextRequest) {
  return withApiErrors(async () => {
    ...
    const { name, maxMailboxes, maxAliasesPerMailbox, quotaMb } =
      (await req.json().catch(() => ({}))) || {};
    const normalized = (name || "").trim().toLowerCase();
    if (!DOMAIN_RE.test(normalized)) {
      return apiError(
        400,
        "name must look like a domain, e.g. mail.example.com",
      );
    }
    try {
      ...
      return Response.json(
        {
          ...
          dnsRecords: buildDnsRecords(normalized, {
            ...
            publicKey: dkim.publicKeyBase64,
          }),
        },
        { status: 201 },
      );
