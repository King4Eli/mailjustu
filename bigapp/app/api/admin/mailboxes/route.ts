...
export async function GET(req: NextRequest) {
  return withApiErrors(async () => {
    ...
    const mailboxes = await Promise.all(
      rows.map(async (row) => ({
        ...row,
        storageUsedBytes: await getMailboxUsageBytes(row.email),
      })),
    );
...
export async function POST(req: NextRequest) {
  return withApiErrors(async () => {
    ...
    if (adminScope.domain && domain !== adminScope.domain) {
      return apiError(
        403,
        `As a domain admin you can only create mailboxes on ${adminScope.domain}`,
      );
    }
    ...
    try {
      ...
      const [[existingAlias]] = await conn.query<RowDataPacket[]>(
        "SELECT 1 FROM virtual_aliases WHERE source = ?",
        [normalizedEmail],
      );
      if (existingAlias) {
        await conn.rollback();
        return apiError(
          409,
          "That address is already an alias, not available as a mailbox",
        );
      }
      ...
      const limit =
        domainRow.max_mailboxes ??
        Number(process.env.MAX_MAILBOXES_PER_DOMAIN) ??
        null;
      if (limit) {
