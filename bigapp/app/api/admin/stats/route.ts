...
export async function GET(req: NextRequest) {
  return withApiErrors(async () => {
    ...
    const [[{ mailboxCount }]] = await pool.query<RowDataPacket[]>(
      "SELECT COUNT(*) AS mailboxCount FROM virtual_users",
    );
    const [[{ domainCount }]] = await pool.query<RowDataPacket[]>(
      "SELECT COUNT(*) AS domainCount FROM virtual_domains",
    );
