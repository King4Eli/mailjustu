...
export async function POST(req: NextRequest) {
  return withApiErrors(async () => {
    ...
    if (source === email) return apiError(400, 'That is already your primary address')
    ...
    try {
      ...
      const [[existingMailbox]] = await conn.query<RowDataPacket[]>('SELECT 1 FROM virtual_users WHERE email = ?', [source])
      if (existingMailbox) {
        await conn.rollback()
        return apiError(409, 'That address is already a mailbox, not available as an alias')
      }
