        ...
        list
          ...
          .map(async (box) => {
            const status = await client
              .status(box.path, { unseen: true, messages: true })
              .catch(() => null);
            return {
              path: box.path,
              name: box.name,
...
export async function POST(req: NextRequest) {
  return withApiErrors(async () => {
    ...
    if (!isValidFolderName(name))
      return apiError(400, "name is required and cannot contain / or \\");
    ...
    try {
      await withImap(email, password, async (client) => {
        if (limit) {
          ...
          if (customCount >= limit) {
            throw Object.assign(
              new Error(
                `You're at the limit of ${limit} custom folders. Delete one before creating another.`,
              ),
              { overLimit: true },
            );
          }
      ...
      return apiError(
        (err as { overLimit?: boolean }).overLimit ? 409 : 500,
...
export async function DELETE(req: NextRequest) {
  return withApiErrors(async () => {
    ...
    try {
      ...
      return apiError(
        (err as { notEmpty?: boolean }).notEmpty ? 409 : 500,
        (err as Error).message,
      );
    }
  });
