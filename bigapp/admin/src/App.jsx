    ...
    <div className="storage-usage">
      ...
      <div className="storage-bar">
        <div
          ...
          style={{
            width: `${pct}%`,
            background: pct > 90 ? "#d3352f" : "#5b4cd9",
          }}
        />
      </div>
    </div>
    ...
    <div className="token-gate">
      <form onSubmit={handleSubmit} className="token-gate-card">
        ...
        <p className="token-hint">
          Needs a mailbox that's either listed in SUPER_ADMIN_EMAILS or has
          is_admin set for its domain.
        </p>
      </form>
    </div>
  );
    ...
    <div className="modal-backdrop" onClick={onCancel}>
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-actions">
          ...
          <button
            className="modal-danger-confirm"
...
function generatePassword(length = 16) {
  ...
  return Array.from(
    values,
    (v) => PASSWORD_CHARS[v % PASSWORD_CHARS.length],
  ).join("");
}
  ...
  return (
    <section className="panel services-panel">
      <div className="panel-head">
        <div>
          <h2>Mailboxes</h2>
          <p>Create and remove virtual mail accounts</p>
        </div>
      </div>
      ...
      <div className="service-table" style={{ marginTop: 20 }}>
        <div
          className="service-row table-head"
          style={{
            gridTemplateColumns: isSuper
              ? "1.6fr .8fr 1.2fr .8fr 32px 32px"
              : "1.6fr .8fr 1.2fr 32px 32px",
          }}
        >
          <span>EMAIL</span>
          <span>CREATED</span>
          <span>STORAGE</span>
          {isSuper && <span>ADMIN</span>}
          <span />
          <span />
        </div>
        ...
        {mailboxes.length === 0 && (
          <div className="empty">No mailboxes yet — create one above.</div>
        )}
      </div>
...
function EditLimitsForm({ domain, defaults, onSave }) {
  ...
  const [maxAliases, setMaxAliases] = useState(
    domain.max_aliases_per_mailbox ?? "",
  );
    ...
    <section className="panel services-panel">
      <div className="panel-head">
        <div>
          <h2>Domains</h2>
          <p>
            Domains this server accepts mail for, with per-domain
            mailbox/alias/storage limits
          </p>
        </div>
      </div>
      ...
      <div className="service-table" style={{ marginTop: 20 }}>
        <div
          className="service-row table-head"
          style={{ gridTemplateColumns: "1.3fr .6fr .8fr .8fr .8fr 32px" }}
        >
          <span>DOMAIN</span>
          <span>MAILBOXES</span>
          <span>MAILBOX LIMIT</span>
          <span>ALIAS LIMIT</span>
          <span>QUOTA</span>
          <span />
        </div>
        {domains.map((domain) => (
          <div key={domain.id}>
            <div
              className="service-row"
              style={{
                gridTemplateColumns: "1.3fr .6fr .8fr .8fr .8fr 32px",
                cursor: "pointer",
              }}
              onClick={() =>
            ...
            >
              ...
              <span>
                {domain.max_mailboxes ??
                  `default (${defaults?.maxMailboxesPerDomain ?? "∞"})`}
              </span>
              <span>
                {domain.max_aliases_per_mailbox ??
                  `default (${defaults?.maxAliasesPerMailbox ?? "∞"})`}
              </span>
              <span>
                {domain.quota_mb
                  ? `${domain.quota_mb} MB`
                  : `default (${defaults?.quotaMb ?? "∞"})`}
              </span>
              {isSuper ? (
                <button
                  className="row-menu"
...
function App() {
  ...
  const filteredServices = useMemo(
    () =>
      (health?.services || []).filter((service) =>
  ...
  async function handleUpdateDomainLimits(
    id,
    maxMailboxes,
    maxAliasesPerMailbox,
    quotaMb,
  ) {
    await api.updateDomainLimits(
      id,
      maxMailboxes,
      maxAliasesPerMailbox,
      quotaMb,
  ...
  const allHealthy =
    health && health.summary.requiredHealthy === health.summary.requiredTotal;
        ...
        <div className="sidebar-bottom">
          <div className="profile">
            ...
            <div>
              ...
              <span>
                {isSuper ? "Super admin" : `Admin · ${session.domain}`}
              </span>
            </div>
            <button
      ...
      <main>
        <header className="topbar">
          ...
          {isSuper && (
            <div className="top-actions">
              <span className="server-select">
                <span className={`status-dot ${allHealthy ? "" : "down"}`} />{" "}
                {config.mailHost}
              </span>
            </div>
          )}
        ...
        <div className="content">
          <section className="page-heading">
            <div>
              <p className="eyebrow">MAIL SERVER</p>
              <h1>{active}</h1>
              <p>Monitor your mail infrastructure and delivery health.</p>
            </div>
          ...
          {loadError && (
            <div
              ...
              style={{ background: "#fde7e7", borderColor: "#f3c9c9" }}
            >
              <div className="health-check" style={{ background: "#d3352f" }}>
                <X size={19} />
              </div>
              <div>
          ...
          {isSuper && health && !loadError && (
            <div
              ...
              style={
                allHealthy
                  ? undefined
                  : { background: "#fef3d9", borderColor: "#f7e2ad" }
              }
            >
              <div
                className="health-check"
                style={allHealthy ? undefined : { background: "#b3790a" }}
              >
                <Check size={19} />
              </div>
              <div>
                ...
                <span>
                  {health.summary.requiredHealthy} of{" "}
                  {health.summary.requiredTotal} required services healthy ·{" "}
                  {health.summary.healthy} of {health.summary.total} total
                </span>
              </div>
              <div className="updated">
                <Clock3 size={15} /> Updated {lastUpdated}
              </div>
            </div>
          )}
            ...
            <>
              <section className="stats-grid">
                <StatCard
                  ...
                  tone="blue"
                  note={`${stats?.domainCount ?? 0} domain(s)`}
                />
                <StatCard
                  icon={Activity}
                  label="Messages scanned"
                  value={stats?.rspamd?.scanned ?? "—"}
                  tone="violet"
                  note={
                    stats?.rspamdAvailable ? "via Rspamd" : "Rspamd not running"
                  }
                />
                <StatCard
                  icon={ShieldX}
                  label="Spam rejected"
                  value={stats?.rspamd?.actions?.reject ?? "—"}
                  tone="amber"
                  note={
                    stats?.rspamdAvailable ? "via Rspamd" : "Rspamd not running"
                  }
                ...
                <StatCard
                  icon={ShieldCheck}
                  label="No action (clean)"
                  value={stats?.rspamd?.actions?.["no action"] ?? "—"}
                  tone="green"
                  note={
                    stats?.rspamdAvailable ? "via Rspamd" : "Rspamd not running"
                  }
              ...
              <section className="panel services-panel">
                <div className="panel-head">
                  <div>
                    <h2>Services</h2>
                    <p>Live TCP health check against each container</p>
                  </div>
                </div>
                <div className="service-table">
                  <div
                    className="service-row table-head"
                    style={{ gridTemplateColumns: "2fr .9fr .8fr" }}
                  >
                    <span>SERVICE</span>
                    <span>STATUS</span>
                    <span>LATENCY</span>
                  ...
                  {filteredServices.length === 0 && (
                    <div className="empty">No services match "{query}".</div>
                  )}
                </div>
              </section>
            </>
