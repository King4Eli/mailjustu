...
try {
  ...
  await conn.query("INSERT IGNORE INTO virtual_domains (name) VALUES (?)", [
    domain,
  ]);
  ...
  if (existing) {
    await conn.query(
      "UPDATE virtual_users SET password = ?, is_admin = TRUE WHERE id = ?",
      [hash, existing.id],
    );
    console.log(
      `Updated ${normalizedEmail}: password reset, is_admin=1 (domain admin for ${domain}).`,
    );
  } else {
    await conn.query(
      "INSERT INTO virtual_users (domain_id, email, password, is_admin) VALUES (?, ?, ?, TRUE)",
      [domainRow.id, normalizedEmail, hash],
    );
    console.log(
      `Created ${normalizedEmail}: is_admin=1 (domain admin for ${domain}).`,
    );
  }
  await conn.commit();
  console.log(
    `For full super-admin access, also add "${normalizedEmail}" to SUPER_ADMIN_EMAILS in ./.env/api.env and recreate the container.`,
  );
} catch (err) {
  await conn.rollback();
