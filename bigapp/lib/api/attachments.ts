...
export const MAX_ATTACHMENTS_PER_MESSAGE = Number(process.env.MAX_ATTACHMENTS_PER_MESSAGE) || 10
...
export async function parseFormAttachments(form: FormData): Promise<ParsedAttachment[]> {
  const files = form.getAll('attachments').filter((f): f is File => f instanceof File)
  if (files.length > MAX_ATTACHMENTS_PER_MESSAGE) {
    throw new AttachmentLimitError(`Too many attachments (max ${MAX_ATTACHMENTS_PER_MESSAGE}).`)
  }
  const maxBytes = MAX_ATTACHMENT_SIZE_MB * 1024 * 1024
  const oversized = files.find((f) => f.size > maxBytes)
  if (oversized) {
    throw new AttachmentLimitError(`Attachment too large (max ${MAX_ATTACHMENT_SIZE_MB} MB per file).`)
  }
  return Promise.all(
    files.map(async (f) => ({
      filename: f.name,
