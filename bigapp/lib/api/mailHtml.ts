...
function inlineCidImages(html: string, attachments: InlineImage[]): string {
  ...
  for (const att of attachments) {
    ...
    result = result.replace(new RegExp(`cid:${escapeRegExp(att.cid)}`, 'gi'), dataUri)
  }
  return result
}
...
// allowlist (no script/iframe/on*, no javascript: URLs).
export function renderSafeHtml(html: string, attachments: InlineImage[]): string {
