...
export interface ComposeDraft {
  ...
  subject: string;
  body: string;
  // Rich-text mirror of `body` from the contentEditable editor.
  html?: string;
  from?: string;
  attachments?: File[];
  // Set when opened from the Drafts folder -- replaces instead of duplicating.
  draftUid?: number;
  draftFolder?: string;
  // Set when replying/forwarding -- threads the outgoing message.
  inReplyTo?: string;
  references?: string[];
  // Set when replying/forwarding -- kept separate from `body` until send.
  quoteHeading?: string;
  quoteBody?: string;
}
...
// Mobile always gets full-screen compose, regardless of the desktop preference.
...
function ToolbarButton({
  ...
  return (
    <button
      type="button"
      title={title}
      // mousedown, not click -- click fires after the editor loses selection
      onMouseDown={(e) => {
        e.preventDefault();
        onRun();
      }}
...
// Suggestion dropdown drawn from mail seen this session, not a real address book.
function RecipientField({
  ...
  // Suggest against whatever's typed after the last comma.
...
export function ComposeModal({
  ...
  useEffect(() => {
    ...
    // contentEditable is uncontrolled -- only pushed here, never per keystroke.
    if (bodyRef.current) {
  ...
  function addFiles(files: FileList | null) {
    if (!files) return;
    // Snapshot now -- the input clears e.target.files right after this returns.
