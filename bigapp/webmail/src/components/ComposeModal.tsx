...
import { X, Minus, Paperclip, Send, ChevronDown, ChevronRight, File as FileIcon, Trash2, Maximize2, Minimize2 } from 'lucide-react'
...
export function ComposeModal({
  ...
  function isEmpty(d: ComposeDraft) {
    return !d.subject.trim() && !d.body.trim() && !d.quoteBody?.trim() && (d.attachments?.length ?? 0) === 0
  }
        ...
        {(draft.quoteBody || draft.quoteHeading) && (
          <div className="shrink-0 border-t" style={{ borderColor: 'var(--border)' }}>
            <button
              ...
              style={{ color: 'var(--text-faint)' }}
            >
              {quoteExpanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
              <span>{quoteExpanded ? 'Hide quoted text' : 'Show quoted text'}</span>
            </button>
            {quoteExpanded && (
              <div
              ...
              >
                {draft.quoteHeading && (
                  <p className="mb-1.5 whitespace-pre-wrap font-medium" style={{ color: 'var(--text-muted)' }}>
                    {draft.quoteHeading}
                  </p>
                )}
                {draft.quoteBody && <pre className="whitespace-pre-wrap font-sans">{draft.quoteBody}</pre>}
              </div>
            )}
          </div>
        )}
