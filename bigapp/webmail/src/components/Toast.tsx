      ...
      {toasts.map((toast) => (
        <div
          ...
          style={{
            ...
            borderColor: toast.type === 'error' ? 'var(--danger)' : 'var(--border)',
            color: 'var(--text)',
          }}
        >
          {toast.type === 'error' ? (
            <XCircle size={16} className="mt-0.5 shrink-0" style={{ color: 'var(--danger)' }} />
          ) : (
            <CheckCircle2 size={16} className="mt-0.5 shrink-0" style={{ color: 'var(--accent)' }} />
          )}
          <span className="min-w-0 flex-1">{toast.message}</span>
          <button onClick={() => onDismiss(toast.id)} style={{ color: 'var(--text-faint)' }}>
            <X size={14} />
          </button>
        </div>
      ))}
