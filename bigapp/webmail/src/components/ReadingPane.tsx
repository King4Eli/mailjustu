...
<style>
  ...
  @media (prefers-color-scheme: dark) {
    body { color: #efeff2; background: #1b1b1f; }
    a { color: #8080ff; }
  }
</style>
...
function MessageBody({ message }: { message: EmailMessage }) {
  if (message.html) {
    return <MailHtmlFrame html={message.html} />;
  }
  return (
    <div
    ...
    <div className="mt-4 flex flex-wrap gap-2">
      {message.attachments.map((attachment) => (
        <button
          ...
          onClick={() =>
            onDownload(message.id, attachment.index, attachment.name)
          }
      ...
      >
        ...
        {isSpamFolder ? (
          <ActionButton
            icon={ShieldCheck}
            label="Not spam"
            onClick={() => onMarkNotSpam(message.id)}
          />
        ) : (
          <ActionButton
            icon={ShieldAlert}
